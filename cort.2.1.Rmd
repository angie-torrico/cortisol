...
final_df$log_mean_uc_psg<-log10(final_df$mean_uc_psg)

final_df$mean_ucp_sg<-mean(final_df$log_mean_uc_psg, na.rm=TRUE)

final_df$sd_mean_uc_psg<-sd(final_df$log_mean_uc_psg, na.rm=TRUE)

final_df$stanmean_ucpsg<-((final_df$mean_uc_psg - final_df$log_mean_uc_psg)-(2*final_df$sd_mean_uc_psg))
```
...
final_df$cort_log<-log10(final_df$cort_numeric.x)
...
final_df$time_poly<-poly(final_df$timess, degree = 2, coefs = NULL, raw = FALSE, simple = FALSE)

```

```{r}
nullmodel1 <- lmer(cort_log ~ stanmean_ucpsg + time_poly + repro_broad +
                     (1|wo_p), data = final_df) 
...
final_df$resid <- resid(nullmodel1)
